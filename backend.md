...
- [x] **S5** — агрегаты (`/api/stats/*`). PR: —
  - `src/routes/stats.ts` — 4 эндпоинта (пп.12–15). Все числа считает Postgres:
    `count(*) FILTER (WHERE …)` в одном проходе по `people`, разрезы по махаллям —
    `GROUP BY mahallas.id`. В JS остались только две демо-формулы (`neetTrend`,
    `monthly`), которым на вход нужны уже посчитанные скаляры, и раскладка
    результатов `GROUP BY` по фиксированному порядку `STATUSES`/`PROGRAMS`.
  - Разрезы по махаллям (`byMahalla`, `/stats/map`) строятся **от таблицы
    `mahallas` через LEFT JOIN**, скоуп людей уезжает в `ON`: пустые махалли
    остаются в ответе с нулями, как во фронте (`index.tsx` рисует все 12
    столбцов). `share` считает SQL: `round(neet*100 / NULLIF(total,0), 1)` с
    `COALESCE(…,0)` — деление на ноль даёт 0, а не NULL.
  - Скоуп общий с S3/S4: `scopeConditions()` из `routes/people.ts` экспортирована
    и переиспользуется всеми агрегатами (`own_mahalla` → своя махалля во всех
    четырёх ответах, `routed_only` → `program IS NOT NULL`, `all_*` → без
    ограничений). Для разрезов по махаллям own_mahalla дополнительно сужает и
    список махаллей (`WHERE mahallas.id = свой`), иначе в ответе остались бы
    11 нулевых строк чужих махаллей.
  - Канбан (п.14) — 4 независимых запроса с `LIMIT 40` каждый (порядок колонок
    из `REVIEW_STATUSES`), а не одна выборка с разбором в JS: обрезка тоже должна
    быть в SQL.
  - Тесты: `test/stats.test.ts` (21) + `seedStatsFixture()` в helpers — 24
    человека в 3 махаллях, остальные 9 пустые. Покрыто: точные KPI, все 7 строк
    `byStatus` (включая нулевые), `byMahalla` с пустой махаллёй, формула тренда,
    состав и порядок `attention`, `share` (в т.ч. 0 при total=0), 4 колонки
    канбана с составом и порядком, множество кейсов канбана (NEET с «Флаг снят»
    входит, не-NEET с «Флаг снят» — нет), числа воронки, `rate` программ (в т.ч.
    `sent=0 → 0`), помесячная формула, 401 без токена и сужение всех четырёх
    ответов для роли махалли и специалиста по занятости. Итого 112/112 зелёных.
  - **Отклонений от спеки нет.** Отмечено расхождение самой спеки с фронтом:
    §3 п.15 считает `programs.ok` по `outcome IN ('Трудоустроен','Учится')`, а
    `analytics.tsx` учитывает ещё `programOutcome IN ('Трудоустроен','Завершил')`.
    Сделано **по спеке** (она — источник правды); если фронт будет ходить в API,
    цифра «успешных» на его странице станет чуть ниже прежней.
